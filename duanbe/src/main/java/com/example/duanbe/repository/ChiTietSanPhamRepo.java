...
public interface ChiTietSanPhamRepo
    ...
    @Query(value = """
            ...
            LEFT JOIN ( SELECT
                        ...
                        -- ✅ TIMEZONE FIX: Convert server time to Vietnam time (UTC+7)
                        AND km.ngay_bat_dau <= SWITCHOFFSET(SYSDATETIMEOFFSET(), '+07:00')
                        AND km.ngay_het_han >= SWITCHOFFSET(SYSDATETIMEOFFSET(), '+07:00')
    ...
    @Query(nativeQuery = true, value = """
            ...
            KhuyenMaiHieuLuc AS (
                ...
                -- ✅ TIMEZONE FIX: Convert DB datetime to Vietnam TZ before comparison
                WHERE CAST(km.ngay_bat_dau AT TIME ZONE 'SE Asia Standard Time' AS DATETIMEOFFSET) <= SWITCHOFFSET(SYSDATETIMEOFFSET(), '+07:00')

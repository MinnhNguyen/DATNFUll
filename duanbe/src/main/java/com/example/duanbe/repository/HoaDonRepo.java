...
public interface HoaDonRepo extends JpaRepository<HoaDon, Integer> {
  ...
  @Query(value = """
              ...
              ORDER BY
      CASE WHEN hd.trang_thai = N'Hoàn thành' THEN tdh.ngay_chuyen END DESC,
      hd.ngay_tao DESC
              """, nativeQuery = true)
  ...
  // ✅ FIXED: Explicit column aliases for HoaDonResponse DTO projection
  @Query(value = """
      SELECT hd.id_hoa_don AS id_hoa_don, hd.ma_hoa_don AS ma_hoa_don,
             hd.id_khach_hang AS id_khach_hang, kh.ho_ten AS ten_khach_hang,
             hd.trang_thai AS trang_thai, hd.id_voucher AS id_voucher,
             vc.ten_voucher AS ten_voucher, hd.sdt AS sdt_nguoi_nhan,
             hd.sdt AS so_dien_thoai, hd.dia_chi AS dia_chi, hd.email AS email,
             hd.tong_tien_truoc_giam AS tong_tien_truoc_giam,
             hd.phi_van_chuyen AS phi_van_chuyen, hd.ho_ten AS ho_ten,
             hd.tong_tien_sau_giam AS tong_tien_sau_giam,
             hd.hinh_thuc_thanh_toan AS hinh_thuc_thanh_toan,
             hd.phuong_thuc_nhan_hang AS phuong_thuc_nhan_hang,
             hd.loai_hoa_don AS loai_hoa_don, hd.ghi_chu AS ghi_chu,
             hd.ngay_tao AS ngay_tao
      FROM hoa_don hd
  ...
  @Query(value = """
      SELECT hd.id_hoa_don AS id_hoa_don, hd.ma_hoa_don AS ma_hoa_don,
             kh.id_khach_hang AS id_khach_hang, kh.ho_ten AS ten_khach_hang,
             hd.ngay_tao AS ngay_tao, hd.ngay_sua AS ngay_sua,
             hd.trang_thai AS trang_thai, vc.id_voucher AS id_voucher,
             vc.ten_voucher AS ten_voucher, hd.sdt AS sdt_nguoi_nhan,
             hd.sdt AS so_dien_thoai, hd.dia_chi AS dia_chi, hd.email AS email,
             hd.tong_tien_truoc_giam AS tong_tien_truoc_giam,
             hd.phi_van_chuyen AS phi_van_chuyen, hd.ho_ten AS ho_ten,
  ...
  // ✅ FIXED: Replace SELECT * with explicit columns for HoaDonResponse projection
  @Query(value = """
      SELECT hd.id_hoa_don AS id_hoa_don, hd.ma_hoa_don AS ma_hoa_don,
             hd.id_khach_hang AS id_khach_hang, kh.ho_ten AS ten_khach_hang,
             hd.ngay_tao AS ngay_tao, hd.ngay_sua AS ngay_sua,
             hd.trang_thai AS trang_thai, hd.id_voucher AS id_voucher,
             vc.ten_voucher AS ten_voucher, vc.ma_voucher AS ma_voucher,
             hd.sdt AS sdt_nguoi_nhan, hd.sdt AS so_dien_thoai,
             hd.dia_chi AS dia_chi, hd.email AS email,
             hd.tong_tien_truoc_giam AS tong_tien_truoc_giam,
             hd.phi_van_chuyen AS phi_van_chuyen, hd.ho_ten AS ho_ten,
             hd.tong_tien_sau_giam AS tong_tien_sau_giam,
             hd.hinh_thuc_thanh_toan AS hinh_thuc_thanh_toan,
  ...
  @Query(value = """
      SELECT DISTINCT hd.ma_hoa_don, hd.ngay_tao, tdh.trang_thai, hd.tong_tien_sau_giam
      			...
      			AND hd.id_khach_hang = :idKH
                  ORDER BY hd.ngay_tao DESC""", nativeQuery = true)
  Page<HoaDonResponse> getAllHDByidKHandTT(@Param("idKH") Integer idKH, @Param("trangThai") String trangThai,
      Pageable pageable);
  ...
  @Query(nativeQuery = true, value = """
      ...
      hoa_don.trang_thai, hoa_don.id_voucher, sdt as sdt_nguoi_nhan, dia_chi,
  ...
  Optional<HoaDon> findByMaHoaDonAndIdKhachHang(@Param("maHoaDon") String maHoaDon,

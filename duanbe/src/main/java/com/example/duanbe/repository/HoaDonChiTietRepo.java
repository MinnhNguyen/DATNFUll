...
public interface HoaDonChiTietRepo extends JpaRepository<HoaDonChiTiet, Integer> {
    ...
    @Query(value = """
            SELECT
                -- Chi tiết hóa đơn (từ query gốc)
                hdct.id_hoa_don_chi_tiet,
                hdct.id_hoa_don,
                hdct.so_luong,
                hdct.don_gia,

                -- Sản phẩm (từ query gốc)
                ctsp.id_chi_tiet_san_pham,
                sp.ma_san_pham,
                sp.ten_san_pham,
                sp.anh_dai_dien as hinh_anh,
                ...
                -- Tồn kho (từ query gốc)
                ctsp.so_luong AS so_luong_ton,
                ctsp.so_luong AS so_luong_ton_kho,
                ctsp.trang_thai AS trang_thai_ctsp,
                sp.trang_thai AS trang_thai_san_pham,

                -- ✅ GIÁ BÁN (KM tốt nhất hoặc giá gốc)
                COALESCE(best_km.gia_sau_giam, ctsp.gia_ban) AS gia_ban
            ...
            -- ✅ KHUYẾN MÃI TỐT NHẤT (giá thấp nhất, tránh duplicate)
            LEFT JOIN (
                SELECT
                    ctkm.id_chi_tiet_san_pham,
                    ctkm.gia_sau_giam,
    ...
    @Query(value = """
            BEGIN TRY
                ...
                -- ✅ STOCK CONCURRENCY FIX: Atomic check to prevent oversell
                UPDATE chi_tiet_san_pham
                SET so_luong = so_luong - @SOLUONG
                WHERE id_chi_tiet_san_pham = @IDCTSP
                  AND so_luong >= @SOLUONG  -- Only update if sufficient stock
                  AND trang_thai = 1;       -- Only active products

                -- Check if update failed (stock insufficient)
                IF @@ROWCOUNT = 0
                BEGIN
                    -- Get product details for error message
                    DECLARE @TenSP NVARCHAR(255);
                    DECLARE @SoLuongTon INT;
                    SELECT @TenSP = sp.ten_san_pham, @SoLuongTon = ctsp.so_luong
                    FROM chi_tiet_san_pham ctsp
                    JOIN san_pham sp ON sp.id_san_pham = ctsp.id_san_pham
                    WHERE ctsp.id_chi_tiet_san_pham = @IDCTSP;

                    DECLARE @ErrorMsg NVARCHAR(500) = FORMATMESSAGE(
                        N'Sản phẩm "%s" chỉ còn %d trong kho, không đủ để bán %d!',
                        @TenSP, ISNULL(@SoLuongTon, 0), @SOLUONG
                    );

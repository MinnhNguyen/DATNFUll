    ...
    <div class="best-selling-products" ref="sectionRef" :class="{ 'visible': isVisible }">
        <div class="container p-0">
            <div class="section-header">
                <h4 class="section-title">Áo sơ mi mới nhập</h4>
...
onMounted(async () => {
    await store.getSanPhamMoiNhat();
    // Chuyển đổi dữ liệu từ API sang định dạng phù hợp với template
    if (store.listSanPhamMoiNhat && store.listSanPhamMoiNhat.length > 0) {
        bestSellingProducts.value = store.listSanPhamMoiNhat.map(item => ({
            id: item.id,
        ...
        console.log('Dữ liệu sản phẩm mới đã chuyển đổi:', bestSellingProducts.value);
    } else {
        console.log('Không có dữ liệu sản phẩm mới từ API');
    }
...
const showProductDetail = (product) => {
    selectedProduct.value = {
        ...
        description: 'Áo thể thao chất liệu thun co giãn, thoáng khí, thấm hút mồ hôi tốt. Thiết kế đơn giản, dễ phối đồ, phù hợp cho các hoạt động thể thao và mặc hàng ngày.'
    };
